...
export function Sidebar({ activeTab, setActiveTab }) {
  ...
  return (
    <aside className="w-64 border-r border-slate-800/80 bg-[#0A0E1A]/90 flex flex-col justify-between shrink-0 min-h-[calc(100vh-4rem)]">
              ...
              <div className="flex items-center space-x-3">
                <Icon
                  className={`w-4 h-4 transition-colors ${
                    isActive ? 'text-cyan-400' : item.color || 'text-slate-500 group-hover:text-slate-300'
                  }`}
                />
      ...
      <div className="p-4 border-t border-slate-800/60 bg-slate-950/40">
        <div className="flex items-center space-x-2 text-[11px] text-slate-400 font-mono">
          ...
          <span>Fencing: Monotonic v1</span>
        </div>
